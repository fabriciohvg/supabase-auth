...
All of the APIs documented below support the `error` parameter for customizing
the error message.
...
default seconds are optional, as are sub-second decimals.
...
Validate standard 48-bit MAC address
[IEEE 802](https://en.wikipedia.org/wiki/MAC_address).
...
This schema will produce `"invalid_format"` issues, which are more descriptive
...
The `.safeExtend()` method works similarly to `.extend()`, but it won't let you
overwrite an existing property with a non-assignable schema. In other words, the
...
{/_ Some general rules of thumb for avoiding circularity _/}
...
An exclusive union (XOR) is a union where exactly one option must match. Unlike
regular unions that succeed when any option matches, `z.xor()` fails if zero
options match OR if multiple options match.
...
**New** — As of v4.2, Zod properly supports numeric keys inside records in a way
that more closely mirrors TypeScript itself. A `number` schema, when used as a
record key, will validate that the key is a valid "numeric string". Additional
numerical constraints (min, max, step, etc.) will also be validated.
...
By default, `z.record()` errors on keys that don't match the key schema. Use
`z.looseRecord()` to pass through non-matching keys unchanged. This is
particularly useful when combined with intersections to model multiple pattern
properties:
...
The `z.property()` API works with any data type (but it's most useful when used
...
This is also useful if you want to pass some input value through some mutating
refinements.
...
To customize this, pass a second generic to `.brand()` to specify the direction
of the brand.
...
<Tabs groupId="lib" items={["Zod", "Zod Mini"]}> <Tab value="Zod"> ```ts
function setCommonNumberChecks<T extends z.ZodNumber>(schema: T) { return schema
.min(0) .max(100); }
